...
export const Identification = () => {
  ...
  const triggerAutosave = async (updatedSpans, isCompleted, isPresent) => {
    ...
    setStatusMsg("Saving...");
    try {
      const response = await fetch("http://localhost:8000/api/identification", {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...
          identification_completed: isCompleted,
  ...
  const handleNavigateNext = async () => {
    if (metaphorPresent === null) return;
  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-[1800px] mx-auto text-slate-800 font-sans">
          ...
          {showHistory && (
            <div className="flex-grow overflow-y-auto h-[220px] mt-3 pr-1 space-y-3 animate-fade-in">
              {context.map((utt) => {
                ...
                const isSpeakerAssistant = utt.speaker === "assistant";
        ...
        <div className="bg-white rounded-3xl border border-slate-200 p-5 shadow-sm flex flex-col transition-all">
          ...
          >
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-slate-500">MIPVU Guidelines</span>
            </div>
            {showMIP ? (
              <ChevronUp className="h-4 w-4 text-slate-400" />
            ) : (
            ...
            <div className="flex-grow overflow-y-auto h-[220px] mt-3 pr-1 text-xs text-slate-600 space-y-3 leading-relaxed animate-fade-in">
              <ol className="space-y-2.5 list-none pl-0">
                <li className="flex gap-2 items-start">
                  ...
                  <span>Read the text to get a general understanding of the meaning</span>
                </li>
                <li className="flex gap-2 items-start">
                  <span className="font-bold text-slate-400 shrink-0 select-none w-5 text-right">2.</span>
                  <span>Determine the lexical units</span>
                </li>
                <li className="flex gap-2 items-start">
                  <span className="font-bold text-slate-400 shrink-0 select-none w-5 text-right">3a.</span>
                  <span>Establish the contextual meaning of the unit</span>
                </li>
                <li className="flex gap-2 items-start">
                  <span className="font-bold text-slate-400 shrink-0 select-none w-5 text-right">3b.</span>
                  <span>Determine if it has a more basic meaning</span>
                </li>
                <li className="pl-7">
                ...
                <li className="flex gap-2 items-start">
                  ...
                  <span>If yes, mark the unit as metaphorical.</span>
                </li>
              </ol>
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 text-[11px] text-slate-500 italic">
                <strong>Example:</strong> "We're <u>stuck</u> in a loop. We keep repeating the same thoughts and arguments without getting anywhere." The basic meaning of "stuck" (physically fixed/unable to move) contrasts with the contextual meaning of feeling trapped in a repetitive cycle of ideas. Hence, it is metaphorical.
              </div>
            </div>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
        ...
        <div ref={activeCardRef} className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm space-y-6">
