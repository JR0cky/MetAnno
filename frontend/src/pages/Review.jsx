...
export const Review = () => {
  ...
  const filteredUtterances = utterances.filter((u) => {
    ...
    if (filterMode === "started") {
      return matchesSearch && u.identification_completed && !u.classification_completed;
    }
    if (filterMode === "completed") {
      return matchesSearch && u.classification_completed;
    }
    ...
    <div className="space-y-6 animate-fade-in max-w-[1800px] mx-auto text-slate-800">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm flex flex-col lg:flex-row items-center justify-between gap-4">
        ...
        <div className="flex gap-2 w-full lg:w-auto flex-wrap">
          ...
          >
            Not Started ({utterances.filter(u => !u.identification_completed).length})
          </button>
          <button
          ...
          >
            Started / Incomplete ({utterances.filter(u => u.identification_completed && !u.classification_completed).length})
          </button>
          <button
          ...
          >
            Completed ({utterances.filter(u => u.classification_completed).length})
          </button>
        </div>
      </div>
